...
//定时时间0.01  == 10ms
#define		CVI_ANSY_TIME_VAL				0.002
...
{
	short	flg;			//标识为。b0 = 1,该变量有效；b0=0，该变量无效	   （由AddTimeOutVarToAnsyTimer管理）
	short	startflg;		//启动标识，1，自动启动，	
	short	Settime;		//超时数（阀值）
	int		* ptimeout;		//超时累加值（外部幅值）
	int		* poverFlg;		//超时标识（累加值大于阀值，置1）
