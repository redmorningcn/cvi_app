...
*
* Arguments   : pch        is a pointer to the Modbus channel 
    ...
    if (pch != (MODBUS_CH *)0) {
        ...
        pUART->cr    = UARTCR_RXE                                     /* Enable Rx, Tx and UART                                  */
                     ...
                     | UARTCR_ENABLE;              
                                                                      ...
                                                                      /* Compute the RTU timeout counts based on baud rate       */
#if MODBUS_RTU_EN > 0
...
void  MB_CommRxTxISR_Handler (CPU_INT08U  port_nbr)
{
    ...
    pch = &MB_ChTbl[0];
    for (ch = 0; ch < MODBUS_MAX_CH; ch++) {                   /* Find the channel assigned to this port       */
...
void  MB_RTU_TmrInit (CPU_INT32U  freq)
{
    ...
    TIMER3->load                           = BSP_LH79520_CLK / 16 / freq;
