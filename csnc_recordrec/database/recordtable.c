...
int	CreateModelToTableMap(stcModelTableCtrl * stable)
{
	...
	DBMapColumnToShort (stable->map, "MODEL_NUM",&stable->sMode.ModelNum,	&tmp64);						//油箱模型编号
	DBMapColumnToShort (stable->map, "POT_NUM",  &stable->sMode.PotNum,		&tmp64);						//模型点数
	DBMapColumnToShort (stable->map, "SET_OIL",  &stable->sMode.StartOil, 	&tmp64);						//模型初始值
	
	for(i = 0;i < 100;i++)						   
	{
		sprintf(tmpbuf,"HIG%02d",i);
		resultcode = DBMapColumnToShort (stable->map, tmpbuf, &stable->sMode.sModPot[i].HigVal, &tmp64);	//高度
		
		sprintf(tmpbuf,"OIL%02d",i);
		resultcode = DBMapColumnToShort (stable->map, tmpbuf, &stable->sMode.sModPot[i].DipVal, &tmp64);	//油量
	}	
...
////////////////////////////////////////////////////////////////数据发送
		...
		{
			sCsncWr.databuf	= sCtrl.PC.wr;					//接收数据区
			sCsncWr.datalen	= (unsigned char)sCtrl.PC.ConnCtrl.sendlen;	//发送数据区长度
			sCsncWr.rxtxbuf 	= (uint8 *)sendbuf;			//

			sCsncWr.sourceaddr= sCtrl.PC.ConnCtrl.sourceaddr;//地址
			sCsncWr.destaddr  = sCtrl.PC.ConnCtrl.DestAddr;
			sCsncWr.framnum   = sCtrl.PC.ConnCtrl.SendFramNum;
			sCsncWr.framcode  = sCtrl.PC.ConnCtrl.FrameCode;

			DataPackage_CSNC(&sCsncWr);						//按csnc协议打包

			cvi_SendCom(sCtrl.PC.sCom, sCsncWr.rxtxbuf, sCsncWr.rxtxlen);//通过串口发送数据
...
//----------------------------------------------------------------------------
int	cvi_InitBspDB_(stcThreadModel * T_Ctrl)
{
